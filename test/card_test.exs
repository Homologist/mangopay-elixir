defmodule CardTest do
  ...
  test "update card" do
    use_cassette "#{module_name(__MODULE__)}/card/update" do
  ...
  test "all cards by fingerprints" do
    use_cassette "#{module_name(__MODULE__)}/card/fingerprint/all" do
  ...
  test "all by user" do
    use_cassette "#{module_name(__MODULE__)}/card/user/all" do
  ...
  test "all user by fingerprint" do
    use_cassette "#{module_name(__MODULE__)}/card/user/fingerprint/all" do
...
#      assert length(Poison.decode!(response.body)) > 0
  ...
  test "all transaction by fingerprint" do
    use_cassette "#{module_name(__MODULE__)}/transaction/fingerprint/all" do
      assert {:ok, response} = Mangopay.Card.all_by_fingerprints created_card()["Fingerprint"]
      assert length(Poison.decode!(response.body)) > 0
    end
  end
  ...
  test "deactivate card" do
    use_cassette "#{module_name(__MODULE__)}/card/deactivate" do
